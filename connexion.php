...
if (isset($_POST['formconnexion']))
 {
    ...
    {
        # code...
        $requser =$bdd ->prepare("SELECT * FROM entreprise WHERE  email = ? AND motdepasse = ?");
        $requser->execute(array($email,$motdepasse));
        $userexist = $requser -> rowCount();
        if ($userexist== 1)
        {
    ...
    {
        # code...
        $requser =$bdd ->prepare("SELECT * FROM administration_encadrant WHERE  email = ? AND motdepasse = ?");
        $requser->execute(array($email,$motdepasse));
        $userexist = $requser -> rowCount();
        if ($userexist== 1)
        {
            ...
            $_SESSION['nom']= $userinfo['nom'];
            $_SESSION['email'] = $userinfo['email'];
            header("Location: tableau-de-bord-professeur.php?id=".$_SESSION['id']);
        }
        else {
            $erreur = "Mauvais email ou mot de passe !";
